...
#=
takes in function, input, output, runge-kutta output, step size, returns new step size, new input, value at new input
=#
...
function checkperiod(E, ip, px, px21)
    ...
    for p = 1:px
        ...
        if (d1 < eps) && (d2 < eps) && (abs(t1 - t2) < eps)
            return 1  # Return 1 for periodic behavior
        end
    end
    return 0
end

function integrate_multiple(f; tol = 1e-6, tend = 2000, px = 16, plotting = false)
    n_points = 10
    periods = zeros(Int, n_points)
    fixed_points = 0
    aperiodic = 0
    for i in 1:n_points
        y = [-1+2*rand(),-1+2*rand(),-1+2*rand(),-1+2*rand(),-1+2*rand(),-1+2*rand(),-1+2*rand(),-1+2*rand()]
        ...
        if norm(y) >= 1e4
            periods[i] = 3
        # Check for a fixed point (no oscillations)
        elseif norm(fy) < 0.5*tol && period == 0
            fixed_points += 1
        # If neither periodic nor divergent nor a fixed point
        elseif period != 1
            aperiodic += 1
        end
    end
    ...
    # Calculate fractions
    frac_periods = count(periods .== 1) / n_points
    frac_fixed_points = fixed_points / n_points
    frac_aperiodic = aperiodic / n_points

    # Return the appropriate result
    if frac_periods > 0.5
        return 1
    elseif frac_fixed_points > 0.8
        return 2
    elseif frac_aperiodic > 0.8
        return 0
    else
        return 3
